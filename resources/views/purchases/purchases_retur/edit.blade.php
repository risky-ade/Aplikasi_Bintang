...
<div class="content-wrapper">
  <div class="content-header">
    <div class="container-fluid">
      <h3 class="mb-3">Edit Retur Pembelian</h3>
    </div>
  </div>

  <section class="content">
    <div class="container-fluid">
      <form method="POST" action="{{ route('retur-pembelian.update', $retur->id) }}">
        @csrf
        @method('PUT')
        <div class="card">
          <div class="card-body">
            ...
            <div class="row mb-3">
              ...
              <div class="col-md-4">
                ...
                <input type="text" class="form-control" value="{{ $retur->pembelian->no_faktur ?? '-' }} - {{ $retur->pembelian->pemasok->nama ?? '-' }}" readonly>
              </div>
              <div class="col-md-4">
              ...
              <tbody>
                @foreach ($pembelianDetails as $index => $detail)
                  @php
                    ...
                    $qtyBeli = (int) $detail->qty;
                    $qtyReturLain = (int) ($returLainnya[$produkId] ?? 0);
                    $maxRetur = max(0, $qtyBeli - $qtyReturLain);
                    $qtyValue = old('qty_retur.' . $index, $existing->qty_retur ?? 0);
                    $diskonUnit = (float) ($detail->diskon ?? 0);
                    $subtotal = max(0, (int) $qtyValue * ((float) $detail->harga_beli - $diskonUnit));
                  @endphp
                  <tr>
                    <td>
                    ...
                    <td>
                      <input type="number" name="qty_retur[]" class="form-control qty-retur" min="0" max="{{ $maxRetur }}" value="{{ $qtyValue }}" data-harga="{{ $detail->harga_beli }}" data-discunit="{{ $diskonUnit }}">
                    </td>
